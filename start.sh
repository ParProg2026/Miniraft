...
go run ./raftserver "$NODE_1" "$PEERS_FILE" > node1_stdout.log 2>&1 &

echo "[Cluster Manager] Booting Raft Node 2 ($NODE_2)..."
go run ./raftserver "$NODE_2" "$PEERS_FILE" > node2_stdout.log 2>&1 &

echo "[Cluster Manager] Booting Raft Node 3 ($NODE_3)..."
go run ./raftserver "$NODE_3" "$PEERS_FILE" > node3_stdout.log 2>&1 &

echo "[Cluster Manager] Booting Raft Node 4 ($NODE_3)..."
go run ./raftserver "$NODE_4" "$PEERS_FILE" > node4_stdout.log 2>&1 &

echo "[Cluster Manager] Booting Raft Node 5 ($NODE_3)..."
go run ./raftserver "$NODE_5" "$PEERS_FILE" > node5_stdout.log 2>&1 &
