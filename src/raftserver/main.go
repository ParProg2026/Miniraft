...
// startDebugCLI runs a blocking loop reading from standard input.
// It allows developers to introspect and control the node state at runtime.
func startDebugCLI(state *RaftServer) {
	scanner := bufio.NewScanner(os.Stdin)
	for scanner.Scan() {
		debugCommand := scanner.Text()
		switch debugCommand {
		case "log":
