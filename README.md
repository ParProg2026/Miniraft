...
- **`raftserver/`**: Contains the Raft server implementation.
...
- **`raftclient/`**: Contains the Raft client implementation.
...
- **`start.sh`**: Orchestration script to launch a 3-node local cluster and an interactive client.
- **`config/peers.txt`**: Configuration file listing the addresses of all nodes in the cluster.
- **`servers.txt`**: Alternative server list (if applicable).
...
2. **Manual Execution**:
   - To start a server:
     ```bash
     go run ./raftserver <host:port> <peers_file>
     ```
   - To start a client:
     ```bash
     go run ./raftclient <server_host:port>
     ```
